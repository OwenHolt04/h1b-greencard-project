import React from 'react';
import {
  useCurrentFrame, useVideoConfig, interpolate, spring, AbsoluteFill, Easing,
} from 'remotion';
import { C, CAPTIONS, cardStyle, badgeStyle } from '../lib/constants';
import { FONT_SANS } from '../lib/fonts';
import { MinimalFrame } from '../components/MinimalFrame';
import { Caption } from '../components/Caption';
import { ReadinessScore } from '../components/ReadinessScore';

const FORMS_WITH_SYNC = [
  { code: 'ETA-9089', syncs: true },
  { code: 'I-140', syncs: true },
  { code: 'I-485', syncs: true },
  { code: 'I-765', syncs: false },
  { code: 'I-131', syncs: false },
  { code: 'G-28', syncs: false },
];

/**
 * Beat 5 — Fix + Score Improvement (10s)
 * Two visual phases:
 *   Phase 1 (0-4s): Close-up of the employer name field being fixed.
 *   Phase 2 (4-10s): Wide view — forms sync, score rises 72→80.
 */
export const Beat5_Fix = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  // Phase timing
  const PHASE = {
    fieldHighlight: 0.8 * fps,
    fixApplied: 2.2 * fps,
    phaseTransition: 3.5 * fps,
    formSync: 4.2 * fps,
    scoreRise: 5.0 * fps,
    issueResolved: 6.0 * fps,
    remainingCallout: 7.0 * fps,
    caption: 1.0 * fps,
  };

  const screenEnter = spring({ frame, fps, config: { damping: 200 }, delay: 5 });

  // Phase transition: zoomed in → wide view
  const zoomProgress = interpolate(
    frame,
    [PHASE.phaseTransition, PHASE.phaseTransition + 20],
    [0, 1],
    { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }
  );
  const contentScale = interpolate(zoomProgress, [0, 1], [1.3, 1]);
  const contentOffsetX = interpolate(zoomProgress, [0, 1], [-280, 0]);
  const contentOffsetY = interpolate(zoomProgress, [0, 1], [-60, 0]);

  // Field states
  const fieldHighlight = frame >= PHASE.fieldHighlight && frame < PHASE.fixApplied;
  const fieldFixed = frame >= PHASE.fixApplied;

  // Fix flash: brief white-to-green
  const fixFlashProgress = interpolate(
    frame - PHASE.fixApplied,
    [0, 6, 15],
    [0, 1, 0],
    { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }
  );

  // Form sync glow
  const syncActive = frame >= PHASE.formSync;
  const syncGlow = syncActive ? Math.max(0, 1 - (frame - PHASE.formSync) / (2.5 * fps)) : 0;

  // Issue resolved
  const issueResolved = frame >= PHASE.issueResolved;
  const showRemaining = frame >= PHASE.remainingCallout;

  // Score counter
  const scoreValue = interpolate(
    frame - PHASE.scoreRise,
    [0, 1.5 * fps],
    [72, 80],
    { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }
  );

  return (
    <AbsoluteFill>
      <MinimalFrame activeScreen="Intake">
        <div
          style={{
            display: 'flex',
            gap: 20,
            padding: '20px 28px',
            height: '100%',
            opacity: interpolate(screenEnter, [0, 1], [0, 1]),
            transform: `scale(${contentScale}) translate(${contentOffsetX}px, ${contentOffsetY}px)`,
            transformOrigin: '25% 35%',
          }}
        >
          {/* LEFT — Shared Data with fix highlight */}
          <div style={{ width: 380, flexShrink: 0 }}>
            <div style={{ fontSize: 18, fontWeight: 700, color: C.slate800, marginBottom: 10 }}>
              Shared Case Data
            </div>
            <div style={{ ...cardStyle, padding: '14px 16px' }}>
              {[
                { label: 'Full Name', value: 'Yuto Sato', type: null },
                {
                  label: 'Employer Legal Name',
                  value: fieldFixed ? 'Helix Systems, Inc.' : 'Helix Systems Inc.',
                  type: fieldFixed ? 'fixed' : fieldHighlight ? 'highlight' : null,
                },
                { label: 'SOC Code', value: '15-1256.00', type: 'warning' },
                { label: 'Salary', value: '$165,000', type: 'warning' },
                { label: 'H-1B Expires', value: 'Jul 15, 2025', type: null },
                { label: 'Travel Dec 2023', value: 'Incomplete', type: 'info' },
              ].map((f, i) => {
                let bg = 'transparent';
                let valueColor = C.slate700;
                let icon = null;

                if (f.type === 'highlight') {
                  bg = C.amber50;
                  valueColor = C.amber600;
                } else if (f.type === 'fixed') {
                  const flashWhite = `rgba(255,255,255,${fixFlashProgress * 0.6})`;
                  bg = fixFlashProgress > 0 ? flashWhite : C.green50;
                  valueColor = C.green700;
                  icon = '\u2713';
                } else if (f.type === 'warning') {
                  bg = C.amber50;
                  valueColor = C.amber600;
                } else if (f.type === 'info') {
                  bg = C.blue50;
                  valueColor = C.blue600;
                }

                return (
                  <div
                    key={i}
                    style={{
                      display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                      padding: '8px 8px',
                      borderBottom: `1px solid ${C.slate100}`,
                      background: bg,
                      borderRadius: 4,
                      marginBottom: 2,
                    }}
                  >
                    <span style={{ fontSize: 16, color: C.slate500 }}>{f.label}</span>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 16, fontWeight: 600, color: valueColor }}>{f.value}</span>
                      {icon && <span style={{ fontSize: 18, color: C.green600, fontWeight: 700 }}>{icon}</span>}
                    </div>
                  </div>
                );
              })}
            </div>

            {/* Fix action / result */}
            {!fieldFixed && fieldHighlight && (
              <div
                style={{
                  marginTop: 12, ...cardStyle, padding: '12px 16px',
                  background: C.accent, color: C.navy900,
                  fontSize: 16, fontWeight: 600, textAlign: 'center',
                  borderColor: C.accent,
                  boxShadow: `0 0 0 4px ${C.accent}30`,
                }}
              >
                Fix: Standardize to &quot;Helix Systems, Inc.&quot;
              </div>
            )}
            {fieldFixed && (
              <div
                style={{
                  marginTop: 12, ...cardStyle, padding: '12px 16px',
                  background: C.green50, color: C.green700,
                  fontSize: 16, fontWeight: 600, textAlign: 'center',
                  borderColor: C.green500,
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                }}
              >
                <span style={{ fontSize: 18 }}>{'\u2713'}</span> Employer name standardized
              </div>
            )}

            {/* Remaining issues callout */}
            {showRemaining && (() => {
              const calloutEnter = spring({ frame: frame - PHASE.remainingCallout, fps, config: { damping: 20, stiffness: 200 } });
              return (
                <div
                  style={{
                    marginTop: 12, ...cardStyle, padding: '12px 16px',
                    borderLeft: `3px solid ${C.amber500}`,
                    background: C.amber50,
                    opacity: interpolate(calloutEnter, [0, 1], [0, 1]),
                    transform: `translateY(${interpolate(calloutEnter, [0, 1], [10, 0])}px)`,
                  }}
                >
                  <div style={{ fontSize: 14, fontWeight: 600, color: C.amber600 }}>
                    SOC/wage review still required — attorney flagged
                  </div>
                </div>
              );
            })()}
          </div>

          {/* CENTER — Form Cards with sync */}
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 18, fontWeight: 700, color: C.slate800, marginBottom: 10 }}>
              {syncActive ? '3 forms synced automatically' : 'Filing Package'}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {FORMS_WITH_SYNC.map((form, i) => {
                const isSyncing = syncActive && form.syncs;
                return (
                  <div
                    key={i}
                    style={{
                      ...cardStyle,
                      padding: '16px 18px',
                      borderColor: isSyncing ? C.accent : C.slate200,
                      borderWidth: isSyncing ? 2 : 1,
                      boxShadow: isSyncing
                        ? `0 0 0 ${syncGlow * 5}px ${C.accent}25, 0 1px 3px rgba(0,0,0,0.06)`
                        : cardStyle.boxShadow,
                    }}
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{ fontSize: 18, fontWeight: 700, color: C.slate800 }}>{form.code}</div>
                      {isSyncing && <div style={badgeStyle(C.green50, C.green600)}>{'\u2713'} Updated</div>}
                      {!form.syncs && <span style={{ fontSize: 14, color: C.slate400 }}>No changes</span>}
                    </div>
                    {isSyncing && (
                      <div style={{ fontSize: 14, color: C.accent, marginTop: 6, fontWeight: 500 }}>
                        Employer name synced from shared record
                      </div>
                    )}
                  </div>
                );
              })}
            </div>
          </div>

          {/* RIGHT — Readiness Score */}
          <div style={{ width: 240, flexShrink: 0 }}>
            <div style={{ ...cardStyle, padding: '24px', textAlign: 'center' }}>
              <div style={{ fontSize: 16, fontWeight: 700, color: C.slate700, marginBottom: 16 }}>
                Filing Readiness
              </div>
              <ReadinessScore
                fromScore={72}
                toScore={80}
                animDelay={PHASE.scoreRise}
                animDuration={Math.round(1.5 * fps)}
                size={200}
                label=""
                popOnChange
              />

              <div style={{ marginTop: 16, fontSize: 14, color: C.slate500, lineHeight: 1.4 }}>
                {issueResolved ? '1 issue resolved. 2 remaining.' : '3 issues pending review'}
              </div>

              {/* Issue summary */}
              <div style={{ marginTop: 14, textAlign: 'left' }}>
                {[
                  { label: 'Employer Name', resolved: issueResolved },
                  { label: 'SOC / Wage', resolved: false },
                  { label: 'Travel History', resolved: false },
                ].map((item, i) => (
                  <div
                    key={i}
                    style={{
                      display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0',
                      fontSize: 14,
                      color: item.resolved ? C.green600 : C.slate500,
                      textDecoration: item.resolved ? 'line-through' : 'none',
                    }}
                  >
                    <span style={{ fontSize: 12 }}>{item.resolved ? '\u2713' : '\u25CB'}</span>
                    {item.label}
                  </div>
                ))}
              </div>
            </div>
          </div>
        </div>
      </MinimalFrame>

      <Caption
        text={CAPTIONS.beat5.main}
        subtext={CAPTIONS.beat5.sub}
        delay={PHASE.caption}
      />
    </AbsoluteFill>
  );
};
