...
export default function Impact() {
  ...
  return (
    ...
    >
        ...
        {metricEntries.map((m) => (
          ...
          >
            ...
            <div className="flex items-center justify-center gap-1.5 mb-1">
              <span className="text-[13px] text-slate-400 line-through">{m.before}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        >
          <div className="px-5 py-4 bg-green-50/50 border-b border-green-100 flex items-center gap-2">
            ...
            <h2 className="text-[15px] font-semibold text-green-900">What CaseBridge Solves</h2>
          </div>
            ...
            {solvesDirect.map((s, i) => (
              <motion.div key={i} variants={item} className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-slate-500 leading-relaxed">{s.detail}</p>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        >
          <div className="px-5 py-4 bg-slate-50 border-b border-slate-200 flex items-center gap-2">
            ...
            <h2 className="text-[15px] font-semibold text-slate-700">Requires Broader Reform</h2>
          </div>
          <div className="p-5 space-y-4">
            {requiresReform.map((r, i) => (
              <motion.div key={i} variants={item} className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-[13px] font-semibold text-slate-600">{r.title}</p>
                  <p className="text-[12px] text-slate-400 leading-relaxed">{r.detail}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      >
        <h3 className="text-sm font-semibold text-slate-900 mb-4 text-center">The CaseBridge Approach</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="text-center">
            ...
            <p className="text-[13px] font-semibold text-slate-800 mb-1">Enter Once</p>
            <p className="text-[12px] text-slate-500">Shared intake populates 6+ forms automatically, eliminating re-keying errors.</p>
          </div>
          <div className="text-center">
            ...
            <p className="text-[13px] font-semibold text-slate-800 mb-1">Validate Early</p>
            <p className="text-[12px] text-slate-500">Pre-submission checks catch contradictions that would surface months later as RFEs.</p>
          </div>
          <div className="text-center">
            ...
            <p className="text-[13px] font-semibold text-slate-800 mb-1">Stay Aligned</p>
            <p className="text-[12px] text-slate-500">Every stakeholder sees the same case from their perspective — no separate truths.</p>
          </div>
        </div>
