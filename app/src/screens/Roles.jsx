  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-lg font-semibold text-slate-900">Stakeholder Views</h1>
          <p className="text-sm text-slate-500">Same case, same record — different priorities and language.</p>
        </div>
        <div className="text-right">
          <p className="text-[10px] text-slate-400 uppercase tracking-wider font-medium">Case</p>
          <p className="text-sm font-medium text-slate-700">{caseRecord.caseName}</p>
        </div>
      </div>
            ...
            <div className="px-6 py-5">
              <div className="flex items-start justify-between gap-6">
                <div className="flex-1">
                  ...
                  <h2 className="text-xl font-semibold text-slate-900 mb-2">{content.greeting}</h2>
                  <p className="text-[14px] text-slate-600 leading-relaxed">{content.statusSummary}</p>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 mb-5">
                ...
                <p className="text-[14px] text-slate-700 leading-relaxed mb-4">{content.nextStep}</p>

                {/* Timeline context */}
                <div className="bg-slate-50 rounded-lg px-4 py-3">
                  <p className="text-[12px] text-slate-500 leading-relaxed">{content.timeline}</p>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5">
                ...
                <div className="space-y-2">
                  {content.actions.map((action, i) => (
                    ...
                    >
                      <span className="text-[13px] font-medium text-slate-700">{action}</span>
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      <div key={item.phase} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                        <div>
                          <p className="text-[13px] font-medium text-slate-700">{item.phase}</p>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="text-[13px] font-semibold text-slate-900">{item.cost}</span>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h4 className={`text-[13px] font-semibold ${config.titleColor} mb-1`}>{alert.title}</h4>
                        <p className="text-[12px] text-slate-600 leading-relaxed">{alert.body}</p>
                      </div>
                    </div>
                  </div>
                );
